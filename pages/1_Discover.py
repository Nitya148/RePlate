...
if not user.get("verified") and user["role"] == "recipient":
    st.warning(
        "Your organization is pending admin verification — you can browse, but claiming is locked."
    )
    ...
    with list_col:
        for idx, l in enumerate(items, 1):
            ...
            with st.container():
                st.markdown(
                    f"""
<div style="background:white;border:1px solid rgba(42,27,36,0.08);
    ...
    <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div>
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:0.18em;color:#695A62;">
                {l['donor_name']}
            </div>

            <div style="font-family:'Cormorant Garamond',serif;font-size:17px;line-height:1.2;">
                {l['name']}
            </div>
        </div>
    </div>
                ...
                f"""<div class="listing-card">
                    ...
                    <div class="info">
                        ...
                        <div class="meta">
                            ...
                            <span class="{urgent_class}">{dist_str}{label}</span>
                        </div>
                    </div>
                </div>""",
            )
